...
#check the demo of these groups first

table_30 <- tableby(
  as.factor(yr_ab) ~ age_grp + gender_grp + degree_grp + religion_grp + sex_o_grp + sib_grp + visa_grp + gpa_grp + gpa_change_grp, 
  data = data_vis
  )
summary(table_30, 
        title = "Latent By Gender", 
        text = TRUE)
#then look at psych well being and latent?

summary(as.factor(data_vis$YearAbroad))
...
summary(as.factor(data_vis$SI1))
#您是否有过自残的行为？1001 82% no, 145 12% yes, 76 6% wish not to answer
#每十个回答者中就有至少一位
#更何况，对于整个人群来说的一个数据点，对于那位曾经自我伤害的同学来说，这就是生活。希望ta们能找到相关的资源。
summary(as.factor(data_vis$SI2))
#在过去一年中，您自残行为有多频繁
#1077 88% no, 117 10% yes, 27 2% often, 1 wish not to answer
summary(as.factor(data_vis$SI3))
summary(as.factor(data_vis$SI4))
summary(as.factor(data_vis$SI5))
summary(as.factor(data_vis$SI6))
#answered yes, 53, 52, 40, 39, ~4%
