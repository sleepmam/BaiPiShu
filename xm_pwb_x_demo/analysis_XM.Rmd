...
#degree by PWB
(plot32 <- ggplot(data_vis_pwb, aes(x = PWB, y = PWB_val, fill = degree_grp)) + geom_bar(position = "dodge", stat = "summary", fun = mean) + scale_fill_brewer(palette="YlOrBr") +
...
(plot37 <- ggplot(data_vis_pwb, aes(x = PWB, y = PWB_val, fill = gpa_grp)) + geom_bar(position = "dodge", stat = "summary", fun = mean) + scale_fill_brewer(palette="YlOrBr") +
  labs(fill = "GPA分组", x = "心理健康指标", y = "对应分数", title = "心理健康指标（按成绩分组）") + 
  theme(text = element_text(family='STKaiti')))

(plot38 <- ggplot(data_vis_pwb, aes(x = PWB, y = PWB_val, fill = gpa_change_grp)) + geom_bar(position = "dodge", stat = "summary", fun = mean) + scale_fill_brewer(palette="YlOrBr")+
...
#"ServSat1", "ServSat2", "ServSat3", "ServSat4", "ServSat5", "ServSat6", "ServSat7", "ServSat8", "ServSat9"
#"咨询疗效", "理解倾听", "保密性", "手续简明", "良好沟通", "尊重我", "语言", "时间冲突", "文化适配"
head(data_vis_mhsat$Service)
...
data_vis_mhsat$ServSat <- recode_factor(data_vis_mhsat$ServSat, 'ServSat1' = "咨询疗效", 'ServSat2' = "理解倾听", 'ServSat3' = "保密性", 'ServSat4' = "手续简明", 'ServSat5' = "良好沟通", 'ServSat6' = "尊重我", 'ServSat7' = "语言", 'ServSat8' = "时间冲突", 'ServSat9' = "文化适配")

#needs error bar
(plot40 <- ggplot(data_vis_mhsat, aes(x = ServSat, y = Sat_val, fill = Service)) + geom_bar(position = "dodge", stat = "summary", fun = mean) + coord_flip()+ scale_fill_brewer(palette="YlOrBr")+
